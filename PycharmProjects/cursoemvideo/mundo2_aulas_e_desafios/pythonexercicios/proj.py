...
t = turtle.Screen()
x = turtle.Turtle()
x.shape('turtle') # nome do objecto
x.color('green') # cor do objecto
"""  # x.rt(90)    # virar para direita
# x.forward(100) # andar para frente
# x.left(90) # virar para esquerda
# x.forward(100)
"""
for i in range(120):
   for c in colors:
...
"""def arvore(x, y, largura, pensize, cor, angle):
    turtle.up()
    turtle.goto(x, y)
    turtle.seth(angle)from random import random"""
"""
altura: int 
...
def criar_tartarugas(colors):
    tartarugas = []
    espacamento = largura // (len(colors) + 10)
    for i, color in enumerate(colors):
        velocidade = turtle.Turtle()
        velocidade.color(color)
        velocidade.shape('turtle')  # escolher tartaruga
        velocidade.lt(90)
        velocidade.penup()
        velocidade.setpos(-largura // 2 + (i + 1) * espacamento, - altura // 2 + 20)
        velocidade.pendown()
    tartarugas.append(velocidade)

    return tartarugas


def veloci(colors):
    tartarugas = criar_tartarugas(colors)
...
def criar_tartarugas(colors):
    tartarugas = [ ]
    espacamento = largura // (len(colors) + 1)
    for i , color in enumerate(colors):
        velocidade = turtle.Turtle()
        velocidade.color(color)
        velocidade.shape('turtle') # escolher tartaruga
