...
def criar_tela_com_linha():
    ...
    for i in range(linhas): # para cada i inteiro em um conjunto de numero _ "linhas"
        ...
        linha.sety((-altura / 2 + (altura / 2) / 2) - (-altura / 2 + (altura / 2) / 2) * (bordo / 100))  # posição da linha coordenada,y)
...
def criar_tartarugas(COLORS):
    ...
    for i, color in enumerate(COLORS): # 
        posicao_inicial_tart_eixo_y = (-altura / 2 + (i + 1) * espaco_entre_tart + (((num_de_tartarugas - 2) / 2) + 0.5) * espaco_entre_tart)
        tart = turtle.Turtle()
        tart.color(color)
        tart.shape('turtle')  # apresentar a tartaruga na tela 
        tart.rt(0) # para virar a horizonal
        tart.penup()
        tart.setpos(posicao_tart_xx - posicao_tart_xx * (bordo / 100),posicao_inicial_tart_eixo_y - posicao_inicial_tart_eixo_y * (bordo / 100))
        tart.pendown()
...
def pontuacao(ponto):    
    pont.penup()
    pont.goto(-min(altura, largura) * 0.025 * 2,(altura / 4 + altura / 8))  # corresponde ao posicionamento da palavra "ponto" segundo (x,y) do quadrado
    pont.pendown()
    pont.write(f"Pontos:  \n  {ponto}", font=('Time New Roman', int((min(altura, largura)) / (1 / (min(altura, largura)* 0.025)* (min(altura, largura)))), 'normal'))
    pont.hideturtle()
...
def nome_record(nome): # definimos outra variável porque outra não nos facilita
    ...
    letra.pendown()
    letra.write(f"  Ponto Máximo: \n  {nome}| {ponto_max}", font=('Time New Roman', int((min(altura, largura)) / (1 / (min(altura, largura)
                                                           * 0.025) * (min(altura, largura)))), 'normal'))
    letra.hideturtle()
...
def recuar():
    ...
    for i, tartaruga in enumerate(tartarugas): # para cada tartaruga dentro da lista enumerada  
        posicao_inicial_tart_eixo_y = (-altura / 2 + (i + 1) * espaco_entre_tart + (((num_de_tartarugas - 2) / 2) + 0.5) * espaco_entre_tart) # posicção-y das tartarugas

        tartaruga.setpos(posicao_tart_xx - posicao_tart_xx * (bordo / 100),posicao_inicial_tart_eixo_y - posicao_inicial_tart_eixo_y * (bordo / 100))
        tartaruga.clear()
...
while True:
    while True: # lupe infinito
        ...
        if cor_tart not in COLORS[:num_de_tartarugas]: # se a cor escolhida não estiver dentro desta lista de cores 
    ...
    if op == 2: # se a opção for "2" o jogo vai terminar
        ...
        recuar() # quando as tartarugas chegarem a meta esta função